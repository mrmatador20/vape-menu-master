-- Modificar a edge function process-referral para aceitar verificação de status do pedido
-- e criar trigger para processar referral apenas quando pedido for confirmado

-- Função para processar referral quando pedido for confirmado
...
BEGIN
  ...
  IF (NEW.status IN ('confirmed', 'delivered') 
    ...
    DECLARE
      v_referrer_id UUID;
      v_points_to_award INTEGER := 10; -- 10 pontos por indicação
...
-- Criar trigger que executa ANTES da atualização do pedido
-- Isso permite que modifiquemos NEW.referral_points_awarded antes de salvar
DROP TRIGGER IF EXISTS trigger_process_referral_on_confirmation ON public.orders;

CREATE TRIGGER trigger_process_referral_on_confirmation
  BEFORE UPDATE ON public.orders
  FOR EACH ROW
  EXECUTE FUNCTION public.process_referral_on_confirmation();